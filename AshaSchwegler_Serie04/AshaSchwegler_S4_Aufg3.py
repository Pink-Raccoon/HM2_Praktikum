...
coeff_neu = np.polyfit(x_neu, y_in, x_neu.shape[0] - 1)
print('coeff_neu',coeff_neu,'\n')
foo_neu = np.polyval(coeff_neu,2020)
print('foo_neu',foo_neu,'\n')

plt.figure(2)
plt.grid()
plt.plot(x, np.polyval(coeff_neu, x), zorder=1, label='polyfit')
plt.scatter(x_in, y_in, marker='x', color='r', zorder=2, label='measured')
plt.scatter(2020, np.polyval(coeff_neu, 2020), marker='x', color='fuchsia', zorder=1, label='extrapolated', lw=3)
plt.ylim([-100, 250])
plt.legend()
plt.show()
""" 
